...
module.exports = function getCoreApi(config) {
  ...
  function call(method, url, params, callback) {
    ...
    if (!config.clientID || !config.clientSecret || !config.site) {
      const missingConfigError = new Error('Configuration missing. You need to specify the client id, the client secret and the oauth2 server.');
    ...
    // OAuth2 server call used to retrieve a valid token
      ...
      const authorizationToken = utils
        .getAuthorizationHeaderToken(config.clientID, config.clientSecret);
